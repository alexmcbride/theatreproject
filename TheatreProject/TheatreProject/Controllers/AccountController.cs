...
namespace TheatreProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            // Fetch user from the email address, so we can get the user's name for login.
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null)
                {
                    user = new Member { UserName = model.UserName, Email = model.Email, Joined = DateTime.Now };
        ...
        {
            if (userId == null || code == null || UserManager.FindByIdAsync(userId) == null)
            {
                return View("Error");
            }
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new Member { UserName = model.Email, Email = model.Email, Joined = DateTime.Now };
        ...
        {
#if DEBUG
            // If we have the DB set to recreate each time then the user IDs change everytime we run it. 
            // This causes issues if we're logged in. So if the ID changes between runs then we log out.
            if (User.Identity.IsAuthenticated)
            {
